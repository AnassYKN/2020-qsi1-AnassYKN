...
let scoreWhenAdvantage: (player, player) => score = (advantagedPlayer, winner) => Game(winner);
...
let string_of_score = (score) =>
    switch score {
        | Points(p)     => "( Player One == " ++ string_of_point(p.playerOne) ++ " ) | " ++ "( Player Two == " ++ string_of_point(p.playerTwo) ++ " )"
        | Forty(f)      => (f.player == PlayerOne) ?
                         "( Player One == 40 ) | " ++ "(Player Two == " ++ string_of_point(f.otherPlayerPoint) ++ " )" :
                         "(Player One == " ++ string_of_point(f.otherPlayerPoint) ++ " ) | " ++ "( Player Two == 40 )"
        | Deuce         => "Equality 40"
        | Advantage(a)  => "Advantage to "++ string_of_player(a)
        | Game(g)       => "Winner is " ++string_of_player(g)
    };